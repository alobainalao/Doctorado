...
from config import (n_stencil, phi, order, dt, R, zi_max, activate_fuente,
                    inlet_z_t, inlet_z_t, inlet_a, zo_max, alpha_sum, 
                    zo_min, outlet_a, alpha_r, model, landa, activate_ext)
...
def build_transport_matrix(V, nodes, groups, normals, pho, D, Div_D, eps_M, 
    ...
    if model == "adr":
        coef_identity -= base * sig * landa

    elif model == "mrmt_block":
        coef_identity -= 2 * base * sig * alpha_sum

    elif model != "mrmt_semi":
        raise ValueError(f"Modelo no válido: {model}")
...
def H_vector(H, M_right, gauss_p, groups, nodes, N, operador= None, Qout=0):
    ...
    if activate_ext and operador != "Identity":
        aux -= Qout * gauss_p
...
def C_vector(C, M_right, gauss_f, groups, t, T, N, C_im, pho):
    ...
    if model == "adr" or model == "mrmt_semi":
        fuente = (fuente_C(t, T) + fuente_C(t - dt, T)) * gauss_f
        b[idx] += fuente[idx]
