...
#     CLASE SIMULATION
# ==============================
def update_supertitles(fig_H, fig_V, fig_C, t0):
    """
    Actualiza los títulos principales de las figuras en función del tiempo.

    Parameters
    ----------
    fig_H, fig_V, fig_C : matplotlib.figure.Figure
        Figuras de Head, Velocity y Concentration
    t0 : float
        Tiempo en segundos
    """
    t_hours = t0 / 3600.0
    ...
    return title_H, title_V, title_C


class Simulation:
    def __init__(self, H_list, C_list, Qout):
    ...
    def update(self, step, mask_h):
        self.t += dt 
        for k in range(4): 
            self.H[k], U, self.C[k] = step_time( H=self.H[k], C=self.C[k], 
                                         ...
                                         Qout=self.Qout[k], t=self.t, T=T, mask_h=mask_h) 
...
y_pos  = [0.94, 0.94, 0.94]   # misma altura para las 3 colorbars (puedes cambiarlo)
...
# EXPORTACIÓN MP4
# ==============================
with writer_H.saving(fig_H, "ns/figures/H.mp4", dpi=150), \
     writer_V.saving(fig_V, "ns/figures/V.mp4", dpi=150), \
     writer_C.saving(fig_C, "ns/figures/C.mp4", dpi=150):

    for n in range(frames):
        t0 = n * dt

        # --- física + actualización de campos ---
