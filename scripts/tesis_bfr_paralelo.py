...
def build_H_matrix(nodes, groups, normals, theta, S_s, K, Div_K, eps_M, sig=1, eps_arr= None, operator = None):
    """
...
def build_transport_matrix(V, nodes, groups, normals, pho, D, Div_D, eps_M, gauss_p, sig):
    """
...
def compute_step_worker(args):
    (
        H_k, C_k, Qout_k, t_k,
        A_left, nodes, groups, normals, eps_M, K,
        grad, pho, D_f, A_right, delta_p,
        gauss_p, gauss_f, T
    ) = args

    A_solver = splu(A_left)
    return step_time(H=H_k, C=C_k, nodes=nodes, groups=groups, 
                      normals=normals, A_solver=A_solver, eps_M=eps_M, 
                      K=K, grad=grad, pho=pho, D_f=D_f, A_right=A_right, 
                      delta_p=delta_p, gauss_p=gauss_p, gauss_f=gauss_f, 
                      Qout=Qout_k, t=t_k, T=T)

# ==============================
#     CLASE SIMULATION
# ==============================
class Simulation:
    def __init__(self, H_list, C_list, Qout, shared):
    ...
    def update(self, step):
        ...
        (
            ...
            mask,
            stencils,
            title_H, title_V, title_C,
            im_H, im_V, im_C, quiv_V
        ) = self.shared

        # ======= 1) ejecutar step_time en paralelo =======
        tasks = []
        with ProcessPoolExecutor(max_workers=4) as ex:
            for k in range(4):
                args = (
                    self.H[k], self.C[k], self.Qout[k], self.t,
                    A_left, nodes, groups, normals, eps_M, K,
                    grad, pho, D_f, A_right, delta_p,
                    gauss_p, gauss_f, T
                    )
                tasks.append(ex.submit(compute_step_worker, args))

            results = [task.result() for task in tasks]
            
        # --- después de recuperar results = [task.result() ...] ---
        for k, (H_new, U, C_new) in enumerate(results):
        ...
        title_H.set_text(f"H (t = {self.t:.2f})")
        title_V.set_text(f"Velocidad |U| (t = {self.t:.2f})")
        title_C.set_text(f"C (t = {self.t:.2f})")
...
def main():
    ...
    if pre:
        ...
        pozo_cor    = data["pozo_cor"]
        hU          = data["hU"]

        eps_arr_    = data["eps_arr_"]
        eps_M       = data["eps_M"]
        eps_arr     = data["eps_arr"]
        ...
        gauss_p     = data["gauss_p"]
        gauss_f     = data["gauss_f"]

        H           = data["H"]
        C           = data["C"]
        U           = data["U"]

        xy_grid     = data["xy_grid"]
        xy          = data["xy"]
        I           = data["I"]
        mask        = data["mask"]
    ...
    fig_H, axes_H = plt.subplots(2, 2, figsize=(18, 8))
    fig_V, axes_V = plt.subplots(2, 2, figsize=(18, 8))
    fig_C, axes_C = plt.subplots(2, 2, figsize=(18, 8))
    ...
    for k in range(4):
        ...
        im_H[i][j] = axes_H[i,j].imshow(H0, vmin=298, vmax=310,
        ...
        axes_C[i][j].set_title(f"(Q={Qout[k]})")

    title_H = fig_H.suptitle("Head H (t = 0.00)", fontsize=20, 
                            color="#DCCB7F", y=0.95)
    title_V = fig_V.suptitle("Velocidad |U| (t = 0.00)", fontsize=20,
                            color="#DCCB7F", y=0.95)
    title_C = fig_C.suptitle("Concentración C (t = 0.00)", fontsize=20, 
                            color="#DCCB7F", y=0.95)

    # Datos necesarios para el loop
    figs   = [fig_H, fig_V, fig_C]
    ims    = [im_H[1][1], im_V[1][1], im_C[1][1]]   # imagen de referencia
    y_pos  = [0.05, 0.05, 0.05]   # misma altura para las 3 colorbars (puedes cambiarlo)
    ...
    sim =Simulation(H.copy(), C.copy(), Qout, shared) 
